...
parser = argparse.ArgumentParser(
    description="Proces the output of a OVOD detector to improve results"
)
parser.add_argument(
    "--json_res", required=True, help="Path to the detections of OVOD detector in COCOC_RES format"
)
parser.add_argument(
    "--image_path",
    required=True,
...
def main_known(args):
    ...
    prototypes, cat_name_in_each_index, cat_in_each_index_original = assemble_prototypes(
        args.prototypes, vocab
    )
    ...
    dataloader = DataLoader(
        data, batch_size=256, shuffle=False, num_workers=8, collate_fn=custom_collate_fn
    )
    ...
    for batch in tqdm(dataloader):
        ...
        sum_masked = masked_T.sum(
            dim=1, keepdim=True
        )  # Sum over channels, shape [n_batch, 1, 768]
        count_masked = masks_expanded.sum(
            dim=1, keepdim=True
        )  # Count of selected elements per batch, shape [n_batch, 1, 1]
        ...
        for s, anot in zip(similarity, annotations):
            ...
            indices = [
                cat_in_each_index_original_mapper[c_name]
                for c_name in vocab
                if c_name != cat_name_in_each_index[max_indice]
            ]
